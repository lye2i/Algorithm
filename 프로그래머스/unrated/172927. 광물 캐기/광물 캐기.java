class Solution {
    ...
    public static void search(int cnt, int[] picks, int turn[]) {
        if(cnt == n) {
            int sum = 0, idx = 0;
    ...
    public int solution(int[] picks, String[] minerals) {
        n = Math.min(picks[0]+picks[1]+picks[2], minerals.length/5+(minerals.length%5 > 0 ? 1 : 0));
