...
public class Main {
	...
	static void spreadFire() {
		...
		while(n-- > 0) {
			...
			for(int d=0; d<4; d++) {
				...
				if(dr < 0 || dr >= R || dc < 0 || dc >= C || visitFire[dr][dc] || map[dr][dc] == '#')	continue;
	...
	static boolean movePerson() {
		...
		while(n-- > 0) {
			...
			for(int d=0; d<4; d++) {
				...
				if(dr < 0 || dr >= R || dc < 0 || dc >= C)	return true;
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.print(cnt == 0 ? "IMPOSSIBLE" : cnt);
	}
}