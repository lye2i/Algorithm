...
public class Main {
	...
	static void combination(int idx, int cnt, Person team, boolean visit[], int s, int y) {
		if(cnt == 7) {
		...
		for(int i=idx; i<person.size(); i++) {
			if(!visit[i]) {
				...
				if(map[team.x][team.y] == 'S') {
					map[team.x][team.y] = 'T';
					combination(i, cnt+1, team, visit, s+1, y);
					map[team.x][team.y] = 'S';
				} else {
					map[team.x][team.y] = 'T';
					combination(i, cnt+1, team, visit, s, y+1);
					map[team.x][team.y] = 'Y';
				}
	...
	public static void main(String[] args) throws Exception {
		...
		combination(0, 0, null, new boolean[person.size()], 0, 0);
